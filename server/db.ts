...
import { InsertSuggestion, InsertUser, suggestions, users } from "../drizzle/schema";
...
export async function listSuggestions() {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  return db.select().from(suggestions).orderBy(desc(suggestions.createdAt)).limit(100);
}
...
export async function updateSuggestionStatus(id: number, status: "pending" | "reviewing" | "done" | "rejected") {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
